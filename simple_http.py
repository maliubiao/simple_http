...
def request(url, **kwargs):
    ...
    while redirect:
        ...
        status = res["status"] 
        if res_header.get("Location") or res_header.get("location"):
            new_url = res_header.get("Location", res_header.get("location"))
            if not new_url.startswith("http"):
                new_url = fix_relative_url(url, new_url)
        else:
            break
...
def fix_relative_url(url, new_url):
    d = urlparse(url)
    host = "%s://%s" % (d.get("schema", "http"), d["host"])
    if d.get("port"):
        host = "%s:%s" % d['port'] 
